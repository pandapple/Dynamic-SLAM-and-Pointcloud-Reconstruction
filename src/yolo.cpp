...
void YOLO::initialize(double a, double b, double c, std::string str)
{
	cout << "Net use " << str << endl;
	this->confThreshold = a;
	this->nmsThreshold = b;
	this->objThreshold = c;
	strcpy(this->netname, str.c_str());
...
void YOLO::sigmoid(Mat* out, int length)
{
	...
	cv::Mat YOLO::detect(Mat& frame)
{
	std::vector<cv::Rect> boxes;
	std::vector<int> classIds;
	std::vector<float> confidences;
	return detect(frame, boxes, classIds, confidences);
}

cv::Mat YOLO::detect(Mat& frame, std::vector<cv::Rect>& boxes_out, std::vector<int>& classIds_out, std::vector<float>& confidences_out)
{
	Mat blob;
				...
				for (j = 0; j < num_grid_x; j++)
				{
					...
					if (box_score > this->objThreshold)
					{
						...
						if (max_class_socre > this->confThreshold)
						{
							...
							if(left < 0){left = 0;}
							if(top < 0){top = 0;}
							float width = (int)(w*ratiow);
							float height = (int)(h*ratioh);
							if((left+width) > frame.cols){width = frame.cols - left -1;}
							if((top+height) > frame.rows){height = frame.rows - top -1;}
	...
	boxes_out.clear();
	classIds_out.clear();
	confidences_out.clear();
	
	// Fill mask and output vectors for detected people (classId == 0)
	for (size_t i = 0; i < indices.size(); ++i)
	{
		int idx = indices[i];
		Rect box = boxes[idx];
		if(classIds[idx] == 0 && confidences[idx] > 0.8)
		{
			mask(box).setTo(0);
		}
		// Store all detected boxes for visualization
		boxes_out.push_back(box);
		classIds_out.push_back(classIds[idx]);
		confidences_out.push_back(confidences[idx]);
	}
