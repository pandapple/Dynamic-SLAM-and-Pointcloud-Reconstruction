...
namespace ORB_SLAM2 {
            ...
            mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
    {
        ...
        //YOLO初始化
        yolo_model.initialize(0.5,0.5,0.5,"yolov5l");
    ...
    cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp)
    {
        ...
        // Detect people and get mask and detection boxes
        mask = yolo_model.detect(mImGray, mPersonBoxes, mPersonClassIds, mPersonConfidences);
...
// 改为彩色图像
            if (mbRGB)
                cvtColor(mImGray, mImGray, CV_RGB2BGR);
...
// 改为彩色图像
        ...
        mCurrentFrame = Frame(mImGray, mImDepth, mask, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf,
                              mThDepth);
    ...
    void Tracking::CreateNewKeyFrame()
    {
        ...
        //added by WYL
        pKF->img0 = this->mImGray.clone();
        pKF->img1 = this->mImDepth.clone();
        pKF->mask = this->mask.clone();
